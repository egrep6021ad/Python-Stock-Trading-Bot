...
class Plot:
    def __init__(self, is_save=False, **kwargs):
        # self.__init__(Model)
        try:
...
def plot_importance_Kfold(models, is_save=False):
...
def plot_aucKfold(models, is_save=False):
...
def plot_train_test(acc_train, acc_test, name, legend=('train', 'test') ,is_save=False):
    plt.plot(acc_train)
    plt.plot(acc_test)
    plt.title(name)
    plt.ylabel(name)
...
def plot_precision_recall_curve(model, X_test, y_test, is_save=False):
...
def __gini__(train, test, y_train, y_test, is_plot=True, is_save=False):
    max_len = 30
    gini_trains = []
    gini_tests = []
...
def __giniByUNiqueData__(data, target_name='label', is_plot=True, is_save=False):
    max_len = 30
    ginis = []
    cols = []
...
def __giniKfold__(cv, X, y, is_save=False):
    max_len = 30
